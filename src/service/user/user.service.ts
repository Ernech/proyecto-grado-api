...
export class UserService {
    ...
    async getCandidateWithCvById(candidateId: string) {
        const user:CandidateEntity = await this.candidateRepository.createQueryBuilder('candidate')
        .select([
            'candidate.id',
            'candidate.name',
            'candidate.lastName',
            'candidate.email'
        ]).innerJoinAndSelect('candidate.personalData','personalData')
        .innerJoinAndSelect('candidate.cvData','cvData')
        ...
        .getOne()
        if (!user || user.cvData.length<0 || !user.personalData) {
